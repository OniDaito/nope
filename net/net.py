...
class Net(nn.Module):
    ...
    def __init__(self, splat: Splat, predict_translate=False, predict_sigma=False, max_trans=1.0):
        ...
        self.conv2b = nn.Conv3d(32, 32, 3, stride=1, padding=1)
        csize = conv_size(csize, padding=1, stride=1, kernel_size=3)

        self.conv3 = nn.Conv3d(32, 64, 3, stride=2, padding=1)
        csize = conv_size(csize, padding=1, stride=2, kernel_size=3)

        self.conv3b = nn.Conv3d(64, 64, 3, stride=1, padding=1)
        csize = conv_size(csize, padding=1, stride=1, kernel_size=3)

        self.conv4 = nn.Conv3d(64, 128, 3, stride=2, padding=1)
        csize = conv_size(csize, padding=1, stride=2, kernel_size=3)

        self.conv4b = nn.Conv3d(128, 128, 3, stride=1, padding=1)
        csize = conv_size(csize, padding=1, stride=1, kernel_size=3)

        self.conv5 = nn.Conv3d(128, 256, 3, stride=2, padding=1)
        csize = conv_size(csize, padding=1, stride=2, kernel_size=3)

        self.conv5b = nn.Conv3d(256, 256, 3, stride=1, padding=1)
        csize = conv_size(csize, padding=1, stride=1, kernel_size=3)
